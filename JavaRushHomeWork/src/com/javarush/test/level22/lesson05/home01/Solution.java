...
public class Solution {
    ...
    public String getPartOfString(String string, String threadName) {
        try {
            ...
            int tab2 = string.lastIndexOf('\t');
            String s = string.substring(tab1+1,tab2);
            return s;
        }
        catch (Exception e) {
            if (threadName == FIRST_THREAD_NAME) throw new TooShortStringFirstThreadException();
            if (threadName == SECOND_THREAD_NAME) throw new TooShortStringSecondThreadException();
